...
move_uploaded_file($_FILES["img1"]["tmp_name"],"Uploads/brands/" .time() . '-'.$_FILES["img1"]["name"]);

$sql="update brands set Image=:vimage1 where id=:id";
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper" >
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                     <h2>Design Image Edit/Upload </h2>   
                    </div>
                </div>              
                 <!-- /. ROW  -->
                  <hr />
  ...
  <div class="form">
				  <form name="insert-form" id="contact-form"  method="POST" enctype="multipart/form-data"> 
								...
								<?php 
								$id=intval($_GET['imgid']);
								$sql ="SELECT Name,Image from brands where brands.id=:id";
								$query = $dbh -> prepare($sql);
								$query-> bindParam(':id', $id, PDO::PARAM_STR);
								$query->execute();
                        ...
                        <center>					 
	                      <img src="Uploads/brands/<?php echo htmlentities($result->Image);?>" width="300" height="200" style="border:solid 1px #000" class="input">
				...
				<div class="inputfield">
				  ...
				  <input type="hidden" class="input" value="<?php echo htmlentities($result->Name);?>" ><?php echo htmlentities($result->Name);?>
			   </div>  		
					...
					<div class="inputfield">    
						<input type="button" value="Back" class="btn" onclick="window.location.href = 'vehicle-brands.php';">    
					  </div>
				  </form>
    </div>
  </div>				  
