...
move_uploaded_file($_FILES["img1"]["tmp_name"],"Uploads/disigns/" .time() . '-'.$_FILES["img1"]["name"]);

$sql="update design set Image=:vimage1 where id=:id";
...
<body>
  ...
  <div class="form">
				  <form name="insert-form" id="contact-form"  method="POST" enctype="multipart/form-data"> 
								...
								<?php 
								$id=intval($_GET['imgid']);
								$sql ="SELECT Image from design where design.id=:id";
								$query = $dbh -> prepare($sql);
								$query-> bindParam(':id', $id, PDO::PARAM_STR);
								$query->execute();
                        ...
                        <center>					 
	                      <img src="Uploads/disigns/<?php echo htmlentities($result->Image);?>" width="300" height="200" style="border:solid 1px #000" class="input">
					...
					<div class="inputfield">    
						<input type="button" value="Back" class="btn" onclick="window.location.href = 'design.php';">    
					  </div>
				  </form>
    </div>
  </div>				  
